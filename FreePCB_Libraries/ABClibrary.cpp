...
PinSpacing Draw( bool bVALIDATE=0 );
...
Form1->PinSpacing->Items->Add("0,5");
Form1->PinSpacing->Items->Add("0,65");
Form1->PinSpacing->Items->Add("0,95");
Form1->PinSpacing->Items->Add("1,0");
Form1->PinSpacing->Items->Add("1,27");
Form1->PinSpacing->Items->Add("2,0");
Form1->PinSpacing->Items->Add("2,5");
Form1->PinSpacing->Items->Add("5,0");
Form1->PinSpacing->Items->Add("10");
Form1->PinSpacing->Items->Add("19");
Form1->PinSpacing->Items->Add("25");
Form1->PinSpacing->Items->Add("39");
Form1->PinSpacing->Items->Add("79");
Form1->PinSpacing->Items->Add("100");
Form1->PinSpacing->Items->Add("200");
...
if( CompleteRead == 0 )
        {
        A = ExtractFilePath(Application->ExeName);
        A = ExtractFilePath(A.SubString(1,A.Length()-1));
        A = ExtractFilePath(A.SubString(1,A.Length()-1));
        A = A + "default_cds.cfg";
        ifstream SET (A.c_str());
        AnsiString gstr_1="", gstr_2="";
        //чтение конфигураций из файла default_cds.cfg
        if(FileExists(A))  while (!SET.eof())
                {
                SET.getline(stringchar,sizeof(stringchar));
                A = stringchar;
                A = A.Trim();
                if(A.SubString(1,14) == "parent_folder:")
                        {
                        long n = 14;
                        gstr_1 = ex_str(A,&n);
                        }
                if(A.SubString(1,20) == "full_library_folder:")
                        {
                        long n = 20;
                        gstr_2 = ex_str(A,&n);
                        }
                }
        Edit1->Text = gstr_1 + gstr_2.SubString(2,gstr_2.Length()-1);
        if (Edit1->Text.Length())
                CompleteRead = READ_CATALOG();
        if(CompleteRead == 0)
                {
                ...
                A = ExtractFilePath(A.SubString(1,A.Length()-1));
                A = ExtractFilePath(A.SubString(1,A.Length()-1));
                Edit1->Text = A + "fp_lib\\lib";
                CompleteRead = READ_CATALOG();
                }
        }
while (CompleteRead == 0)
        {
        Form4->Visible = false;
...
//==============================================================================================
PinSpacing Draw( bool bVALIDATE )
{
PinSpacing PIN_SPACING;
PIN_SPACING.txt1 = "";
PIN_SPACING.txt2 = "";
...
if (i != -1)
        ...
        for (int cnt=n_str[i]; cnt<=end_str[i]; cnt++)
                {
                if( bVALIDATE )
                        if( ((p2.x-p1.x)||(p2.y-p1.y)) && ((p4.x-p3.x)||(p4.y-p3.y)) )
                                break;
...
if ( (Form1->Edit3->Text.Length()&& Form1->Edit3->Text != "n pins") ||        //фильтр PINS
     (Form1->Edit4->Text.Length()&& Form1->Edit4->Text != "n holes") )         //фильтр HOLES
    {
    Form1->ListBox1->Clear();
    int np = -1;
    int nh = -1;
    if(Form1->Edit3->Text.Length() && Form1->Edit3->Text != "n pins")
        {
        B = Form1->Edit3->Text;
        AnsiString ss = Str_Float_Format(B);
        np = (int)StrToFloat(ss);
        }
    if(Form1->Edit4->Text.Length() && Form1->Edit4->Text != "n holes")
        {
        B = Form1->Edit4->Text;
        AnsiString ss = Str_Float_Format(B);
...
if (Form1->PinSpacing->Text.Length() != 0 && Form1->PinSpacing->Text != "pin spacing")         //фильтр по расстоянию между пинами
        {
        int VALIDATE = 1;
        int L = Form1->PinSpacing->Text.Length();
        for(int i=Form1->ListBox1->Items->Count-1; i>=0; i--)
                {
                Form1->ListBox1->ItemIndex = i;
                PinSpacing pin_spacing = Draw( VALIDATE );
                if(( pin_spacing.txt1.SubString(1,L) != Form1->PinSpacing->Text ) &&
                   ( pin_spacing.txt2.SubString(1,L) != Form1->PinSpacing->Text ))
                        {
                        if( i%10 == 0)
                                {
                                Form1->Scan->Caption = "Scan footprints: " + AnsiString(i);
                                Application->ProcessMessages();
                                }
...
//
//FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF
if (Form1->FootName->Text.Length() != 0 && Form1->FootName->Text != "name")         //фильтр по имени
        {
...
//
//
void __fastcall TForm1::FootNameChange(TObject *Sender)
{
FILTER();
Draw();
}
...
int __fastcall TForm1::OnWMCopyData(TWMCopyData &Msg)
{
        ...
        Form1->FootName->Text = (char*)Msg.CopyDataStruct->lpData;
        //Application->ProcessMessages();
        return 0;
}
...
void __fastcall TForm1::PinSpacingChange(TObject *Sender)
{
FILTER();
Draw();        
}
