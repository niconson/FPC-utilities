...
double radius;
bool Flag = false;
if (( ABS(*X) > DEFAULTRotate )||( ABS(*Y) > DEFAULTRotate ))
        {
        Flag = true;
        *X = (*X)/(double)DEFAULTRotate;
        *Y = (*Y)/(double)DEFAULTRotate;
        }
if (ABS(*X) < BY_ZERO)
        {
        if (*Y > 0) start_Ang = 90;
...
if (Flag) *X = (*X)*(double)DEFAULTRotate;
if (Flag) *Y = (*Y)*(double)DEFAULTRotate;
}