...
for( int i=0; i<Form1->LB1->Items->Count; i++ )
        ...
        if( A.Pos("(pin ") == 1 )
                ...
                if( pinname.Length() )
                        {
                        WrPin(refDesRef,compValue,patternName,pinptx,pinpty,pinrot,pinlen,pinname,pnmx,pnmy,pnmrot,pindesc,pdscx,pdscy,pdscrot,rotation,isFlippedTrue,x,y,n_obj);
                        pinname = "";
                        n_obj++;
                        }
                B = Extract(A,"(pt ",")");
